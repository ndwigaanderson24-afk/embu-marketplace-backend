...
const Product = {
    ...
    const [result] = await pool.query(
      `INSERT INTO products
        (seller_id, name, description, category, category_id, price, original_price, emoji, image, images_json, wholesale_tiers_json, video,
         weight, fragile, stock, county, hot, is_new_arrival, is_best_rated, made_in_kenya, flash_deal_ends_at, status, low_stock_threshold, has_variants)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [sellerId || null, data.name, data.description || null, data.category || null, data.category_id || null, data.price,
       ...
       data.weight || 1, !!data.fragile, data.stock || 0, data.county || null, !!data.hot, !!data.is_new_arrival, !!data.is_best_rated, !!data.made_in_kenya, data.flash_deal_ends_at || null,
  ...
  async update(id, sellerId, data) {
    ...
    const allowed = ['name', 'description', 'category', 'category_id', 'price', 'original_price', 'emoji',
      'image', 'images_json', 'wholesale_tiers_json', 'video', 'weight', 'fragile', 'stock', 'hot', 'is_new_arrival', 'is_best_rated', 'made_in_kenya', 'flash_deal_ends_at', 'status', 'low_stock_threshold', 'has_variants'];
